    ...
    if cmd := ev['match'].group(1):
        ...
        if not hoshino.priv.check_priv(ev, hoshino.priv.ADMIN):
            ...
            if not match or len(match.groups()) < 2:
                ...
                group_data[group_id]['hour'] = int(match[1])
                group_data[group_id]['minute'] = int(match[2])
        ...
        base64_str = im2base64str(im)
        msg = (
            f'[CQ:image,file={base64_str}]'
            if group_id not in group_data
            or 'cardimage' not in group_data[group_id]
            or not group_data[group_id]['cardimage']
            else f'[CQ:cardimage,file={base64_str}]'
        )
