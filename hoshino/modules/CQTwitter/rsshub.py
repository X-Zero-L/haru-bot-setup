...
async def dowimg(url: str):
    zip_size = 3072
    try:
        img_path = f'{file_path}imgs/'
        if not os.path.isdir(img_path):
            logger.info(f'{str(img_path)}文件夹不存在，已重新创建')
        ...
        try:
            ...
            if (float(len(pic.content) / 1024) > float(zip_size)):
                ...
                if len(file_suffix[1]) > 0:
                    ...
                    pic.headers['Content-Type'] == 'image/jpeg'
                    or pic.headers['Content-Type'] != 'image/png'
                ):
                    filename = f'{name}.jpg'
                else:
                    filename = f'{name}.png'
...
async def zipPic(content, name):
    ...
    logger.info(f'算出的缩小比:{Proportion}')
    # 缩放
    im.thumbnail((w // Proportion, h // Proportion))
    logger.info(f'Resize image to: {w // Proportion}x{h // Proportion}')
    # 把缩放后的图像用jpeg格式保存:
    try:
...
async def checkstr(rss_str: str):
    ...
    for video in doc_video.items():
        ...
        if len(img_path) > 0:
            rss_str = re.sub(
                re.escape(str(video)),
                f'视频封面：{str(MessageSegment.image(pic2b64(Image.open(img_path))))}',
                rss_str,
            )
        else:
...
# 读取记录
def readRss(url):
    with codecs.open(f"{file_path}{url}.json", 'r', encoding='utf-8') as load_f:
