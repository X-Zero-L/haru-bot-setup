...
async def majsoulInfo(bot, ev: CQEvent):
    ...
    if len(args) == 1:
        ...
        sv.logger.info(f"正在查询{nickname}的对局数据")
        if IDdata == -1:
            await bot.send(ev, "没有查询到该角色在金之间以上的对局数据呢~")
        else:
            if len(IDdata)>1:
                message += "查询到多条角色昵称呢~，若输出不是您想查找的昵称，请补全查询昵称\n\n"
            message += printBasicInfo(IDdata[0],"0","4")
        ...
        if IDdata == -1:
            ...
            message += "查询到多条角色昵称呢~，若输出不是您想查找的昵称，请补全查询昵称\n"
            message += printExtendInfo(IDdata[0], room_level,"4")
            await bot.send(ev, message, at_sender=True)
        else:
...
async def RecordInfo(bot, ev: CQEvent):
    ...
    if IDdata == -404:
        await bot.finish(ev, "获取牌谱屋的数据超时了呢，请稍后再试哦~")
    sv.logger.info(f"正在查询{nickname}的牌谱数据")
    if IDdata == -1:
        await bot.finish(ev, "没有查询到该角色在金之间以上的对局数据呢~")
    else:
        message = "\n"
        if len(IDdata) > 1:
            message += "查询到多条角色昵称呢~，若输出不是您想查找的昵称，请补全查询昵称\n"
        message += printRecordInfo(IDdata[0],4)
        await bot.send(ev, message, at_sender=True)
...
async def majsoulInfo(bot, ev: CQEvent):
    ...
    if len(args) == 1:
        ...
        if IDdata == -1:
            ...
            if len(IDdata)>1:
                message += "查询到多条角色昵称呢~，若输出不是您想查找的昵称，请补全查询昵称\n\n"
            message += printBasicInfo(IDdata[0],"0","3")
            await bot.send(ev, message, at_sender=True)
    elif len(args) == 2:
        nickname = args[1]
        sv.logger.info(f"正在查询{nickname}的对局数据")
        ...
        if IDdata == -1:
            ...
            message += "查询到多条角色昵称呢~，若输出不是您想查找的昵称，请补全查询昵称\n"
            message += printExtendInfo(IDdata[0], room_level,"3")
            await bot.send(ev, message, at_sender=True)
        else:
...
async def RecordInfo(bot, ev: CQEvent):
    ...
    if IDdata == -1:
        ...
        if len(IDdata) > 1:
            message += "查询到多条角色昵称呢~，若输出不是您想查找的昵称，请补全查询昵称\n"
        message += printRecordInfo(IDdata[0],3)
        await bot.send(ev, message, at_sender=True)
